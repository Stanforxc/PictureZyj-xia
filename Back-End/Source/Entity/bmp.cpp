...
BYTE* bmp::ConvertRGBToBMPBuffer(BYTE* Buffer, int width, int height, long* newsize)
{
...
BYTE* bmp::ConvertBMPToRGBBuffer(BYTE* Buffer, int width, int height)
{
	// first make sure the parameters are valid
	if ((NULL == Buffer) || (width == 0) || (height == 0))
		return NULL;
...
bool bmp::LoadBMPIntoDC(HDC hDC, LPCTSTR bmpfile)
{
...
bool bmp::SaveBMP(BYTE* Buffer, int width, int height, long paddedsize, LPCTSTR bmpfile)
{
...
BYTE* bmp::LoadBMP(int* width, int* height, long* size, LPCTSTR bmpfile)
{
