                         ...
                         bool bAlphaEnabled )
{
    try
    {
        ...
        {
            //AfxMessageBox( L"Number of channels is not correct" );
           // return false;
        }
...
bool BMPLoader::SaveBMP( const WCHAR* pFileName_i, int nWidth_i, int nHeight_i, BYTE* pbyData_i )
{
    ...
    try
    {
        ...
        stBitmapInfo.bmiHeader.biBitCount = 4 * 8;
        stBitmapInfo.bmiHeader.biSizeImage = nHeight_i * nNewWidth;
        ...
        //for (int nDataSize = 0; nDataSize < nHeight_i * nNewWidth; nDataSize+=1024)
        {
            ...
            fwrite( pbyData_i, 1, ( nHeight_i * nWidth_i * 4 ), fpFile );
        }
        fclose( fpFile );
        return true;
    }
